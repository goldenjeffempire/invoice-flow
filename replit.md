...
- **Dashboard & API Enhancement (Mar 2026):** Dashboard completely rewritten. 4 new JSON API endpoints added: `GET /api/dashboard/summary`, `GET /api/invoices/recent`, `GET /api/payments/recent`, `GET /api/analytics/revenue-expenses` — all require auth, registered in `invoices/urls.py`, implemented in `dashboard_views.py`. Dashboard improvements: actionable insight banners (overdue alert + due-soon warning shown contextually), activity feed section pulling from `ActivityLog` model (last 12 workspace actions with user avatars and timestamps), improved empty states with CTAs for invoices and payments sections, collection rate stat in Business Stats panel, chart empty states with guidance. Notifications panel in `layout_app.html` rebuilt to show real `Notification` model records — typed icons per notification type (payment/invoice/team/info/warning), unread count badge, blue dot on unread items, mark-as-read on click via `markNotifRead()` JS function calling `/notifications/mark-read/<pk>/`, overdue invoice alert always pinned at top, "All caught up!" empty state. `getCsrfToken()` helper added to `app.js` reading from `<meta name="csrf-token">` tag (compatible with `CSRF_USE_SESSIONS = True`).
