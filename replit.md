...
- **UI System:** App shell (layout_app.html). CSS-variable-driven collapsible sidebar (256px ↔ 64px icon-only mode with tooltips), mobile off-canvas drawer with overlay, sticky topbar, workspace switcher dropdown, user profile footer menu, notification dropdown, dark mode toggle, ⌘K global search palette. **Dashboard rebuilt** — `invoices/views/dashboard_views.py` and `templates/pages/dashboard.html` created with KPI cards (monthly revenue, outstanding, overdue count, active clients), 12-month revenue-vs-expenses Chart.js bar chart, invoice status doughnut chart, recent invoices table, due-soon alerts, top clients section, quick stats row, quick action grid, and overdue alert banner. `LOGIN_REDIRECT_URL = '/dashboard/'`. Sidebar logo and all nav items link to dashboard as primary post-login landing. Client detail page (`pages/clients/detail.html`) enhanced with a 12-month per-client revenue bar chart powered by data from the view. All report pages (revenue, cashflow, aging, expense, forecast, tax, profitability) include Chart.js visualizations. Payments overview includes a monthly collections line chart.
