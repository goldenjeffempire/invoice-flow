...
- **Currency Default Audit (Apr 2026):** Comprehensive audit and fix of all USD currency defaults across the platform. Model-level defaults changed from `"USD"` to `"NGN"` for `Payment`, `Client`, `RecurringSchedule`, and `Expense` models (migration `0022`). View-level defaults updated in `client_views.py`, `expense_views.py`, `recurring_views.py`, and `expense_service.py` to use `workspace.currency or 'NGN'` instead of hardcoded `'USD'`. Client create/edit form template (`templates/pages/clients/form.html`) now uses `default_currency` context variable passed from view. Recurring schedule create/edit forms now use `Invoice.CURRENCY_CHOICES` (full list) and `default_currency` context variable instead of a truncated 4-currency list. All 99 tests passing after migration.
- **Report & Expense System Complete Fix (Apr 2026):** Full audit and fix of all report views and expense pages. (1) Revenue report: mapped `summary.*` → `totals.*`, added `chart_labels`/`chart_data` from `revenue_trend`, fixed `by_client` to include `client__id`. (2) Aging report: flattened nested bucket dict to `d31_60`/`d61_90`/`d90_plus` keys, built flat `aging_rows` list. (3) Cashflow report: mapped `cashflow_data` → `monthly_rows`, added `totals`/`chart_inflows`/`chart_outflows`. (4) Tax report: mapped `tax_collected`/`tax_paid` → `totals` dict, added `date_from`/`date_to` to context. (5) Profitability report: complete template rewrite from Tailwind to app CSS system. (6) Expense report template: complete rewrite from Tailwind + hardcoded ₦ to app CSS system with `workspace.currency_symbol`. (7) Invoice builder: `saveAndSend()` action corrected (`'save'` → `'save_and_send'`); inclusive tax mode calculation properly extracts tax from price; billable expense pre-fill added. (8) Expense form: vendor field changed from free-text to `<select>` dropdown; attachment URLs fixed with `{{ MEDIA_URL }}{{ att.file_path }}`; `django.template.context_processors.media` added to settings. (9) Expense list "This Month" KPI fixed. All 99 tests passing throughout.
- **Premium Redesign Phase Complete (Apr 2026):** Comprehensive premium header upgrades across all remaining authenticated pages. Pages upgraded with `page-eyebrow` / `page-title-lg` / `page-sub-premium` hierarchy and `aria-label` back buttons: (1) `invoices/detail.html` — eyebrow "Invoice", linked client name, color-coded due date; (2) `estimates/detail.html` — eyebrow "Estimate", linked client name, expiry date with expired styling; (3) `expenses/expense_detail.html` — full rewrite with premium header, hero amount with icon, bordered detail rows, enhanced attachment cards, action sidebar; (4) `expenses/vendor_list.html` — premium header, clickable rows, `desk-only` Contact column, premium `empty-state` block, `pagination`/`pg-btn` pagination; (5) `expenses/vendor_detail.html` — eyebrow "Vendor", clickable email link; (6) `expenses/category_list.html` — eyebrow "Finance", `page-title-lg`, `page-sub-premium`; (7) `recurring/schedule_detail.html` — eyebrow "Recurring Schedule", linked client name, amount/interval subtitle, icon-enhanced Pause/Resume buttons; (8) `reports/aging.html`, `cashflow.html`, `revenue.html`, `tax.html`, `expense.html`, `profitability.html` — all wrapped in `page-in`, upgraded to eyebrow "Reports" headers, Export CSV button with download icon; (9) `reports/exports.html`, `reports/forecast.html` — eyebrow headers; (10) `payments/detail.html` — eyebrow "Payment", amount in subtitle; (11) `settings.html` — eyebrow "Account", `page-title-lg`; (12) `newsletter/campaign_list.html` — full Tailwind→app CSS conversion with premium header, eyebrow "Marketing", clickable rows, `action-row`, `empty-state`, `pg-btn` pagination; (13) `newsletter/campaign_detail.html` — full Tailwind→app CSS conversion with eyebrow "Marketing", status badges, KPI mini-cards, styled email preview frame; (14) `newsletter/campaign_form.html` — full Tailwind→app CSS conversion with eyebrow header, card-sectioned form. All 99 tests passing throughout.
