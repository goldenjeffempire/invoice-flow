...
def invoice_list(request):
    ...
    if not workspace:
        ...
        return redirect('invoices:onboarding')
...
def invoice_detail(request, invoice_id):
    ...
    invoice = get_object_or_404(
        Invoice.objects.select_related('client', 'created_by').prefetch_related('items', 'activities', 'payments', 'attachments'),
        id=invoice_id,
        workspace=workspace
    )
...
def invoice_send(request, invoice_id):
    ...
    delivery_method = request.POST.get('method', 'email')

    try:
        invoice = InvoiceService.send_invoice(invoice, request.user, delivery_method)
        ...
        if delivery_method == 'email':
            ...
            try:
                ...
                SendGridService.send_invoice_email(invoice, public_url)
            ...
            messages.success(request, "Invoice marked as sent. Share the link with your client.")
        elif delivery_method == 'whatsapp':
            messages.success(request, "Invoice marked as sent. Share via WhatsApp.")
...
def public_invoice_view(request, token):
    ...
    return render(request, "pages/invoices/public_pay.html", context)
